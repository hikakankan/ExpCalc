class ExpSum implements Expression
{
    ...
    public draw = function (g: WYGraphics, pnt: WYPoint, p: boolean, r: number, cf: WYColor, cb: WYColor, cc: WYColor, ps: WYPoint, cs: WYColor, sel: boolean, english: boolean): void {
        var opsize = g.getFontMetrics().stringWidth(" " + this.mtype + " ");
        ...
        var t = this.exp1.getmtype();
        var ah = pnt.y + (size.height - this.exp1.drawsize(g, false, r, english).height) / 2;
    ...
    public drawsize = function (g: WYGraphics, p: boolean, r: number, english: boolean): WYDimension {
        ...
        var d2;
        var d1 = this.exp1.drawsize(g, false, r, english);
        var t = this.exp2.getmtype();
        if (t == "+" || t == "-" || t == "--") {
    ...
    public reduce = function (selpos: string, status: WYCanvasTextBox): Expression {
        if (selpos == "*") {
            ...
            return new ExpSum(this.mtype, this.exp1.reduce(selpos.substr(1), status), this.exp2);
        }
        else if (selpos.length > 0 && selpos[0] == '2') {
            return new ExpSum(this.mtype, this.exp1, this.exp2.reduce(selpos.substr(1), status));
        }
        else {
            return this;
        }
