...
namespace ExpCalc
{
    ...
    class ExpSum : Expression
    {
        ...
        public ExpSum(String op, Expression x, Expression y)
        {
            mtype = op;
            exp1 = x;
            exp2 = y;
        ...
        {
            int opsize = g.getFontMetrics().stringWidth(" " + mtype + " ");
        ...
        public override Expression reduce(String selpos, TextBox status)
        {
            try
            {
                ...
                {
                    return new ExpSum(mtype, exp1.reduce(selpos.Substring(1), status), exp2);
                }
                else if ( selpos.Length > 0 && selpos[0] == '2' )
                {
                    return new ExpSum(mtype, exp1, exp2.reduce(selpos.Substring(1), status));
                }
                else
                {
                    return this;
