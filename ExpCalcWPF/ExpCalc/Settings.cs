    ...
    class Settings
    {
        ...
        private MVColor get_setting_color(string section, string key, MVColor val, XmlDocument doc)
        {
            try
            {
                ...
                if ( setting != "" )
                {
                    int rgb = Convert.ToInt32(setting, 16);
        ...
        private void set_setting_color(string section, string key, MVColor val, XmlDocument doc)
        {
            try
            {
                ...
                int g = val.WColor.G;
                int b = val.WColor.B;
                set_setting_string(section, key, Convert.ToString(r << 16 | g << 8 | b, 16), doc);
            }
            catch ( Exception )
            {
            }
        ...
        public void load_settings(string path)
        {
            try
            {
                ...
                view_settings.CalcAreaFrameSelectedBackColor = get_setting_color("color", "calcframeselback", view_settings.CalcAreaFrameSelectedBackColor, doc);

                view_settings.AsciiMode = get_setting_bool("view", "asc", view_settings.AsciiMode, doc);
        ...
        public void save_settings(string path)
        {
            try
            {
                ...
                set_setting_color("color", "calcframeselback", view_settings.CalcAreaFrameSelectedBackColor, doc);

                set_setting_bool("view", "asc", view_settings.AsciiMode, doc);
